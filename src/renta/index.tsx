                            ...
                            {effectiveSections.length > 1 && (() => {
                                ...
                                return (
                                    <tr className={`border-b border-gray-200 ${isSubtract ? 'bg-red-50/30' : 'bg-emerald-50/30'}`}>
                                        <td className={`${T.totalCell} border-t-2 border-t-gray-200 ${showClassificationColumns ? '' : T.vline}`}>
                                            <span className={`${T.footerLabel} text-xs`}>{label}</span>
                                        </td>
                                        {showClassificationColumns && <><td className={`${T.cellCompact} border-t-2 border-t-gray-200`} /><td className={`${T.cellCompact} border-t-2 border-t-gray-200 ${T.vline}`} /></>}
                                        {showVariableColumn && !showClassificationColumns && <td className={`${T.cellCompact} border-t-2 border-t-gray-200 ${T.vline}`} />}
                                        {monthsArray.map((p, mi) => {
                                            ...
                                            const vline = mi < monthsArray.length - 1 ? T.vline : ''
                                            return (
                                                <td key={p.id} className={`${T.totalCell} text-right border-t-2 border-t-gray-200 ${vline}`}>
                                        ...
                                        <td className={`${T.actionCol} border-t-2 border-t-gray-200`} />
                                    </tr>
                                )
                            })()}
                        </React.Fragment>
                    ...
                    return (
                        <>
                            ...
                            <tr className="border-b border-gray-100 bg-amber-50/30 group/rv">
                                ...
                                {showClassificationColumns && <><td className={`${T.cellCompact} border-t-2 border-t-gray-200`} /><td className={`${T.cellCompact} border-t-2 border-t-gray-200 ${T.vline}`} /></>}
                                {showVariableColumn && !showClassificationColumns && <td className={`${T.cellCompact} text-center border-t-2 border-t-gray-200 ${T.vline}`}><span className={T.empty}>—</span></td>}
                                {monthsArray.map((p, mi) => {
                                    ...
                                    const vline = mi < monthsArray.length - 1 ? T.vline : ''
                                    return (
                                        <td key={p.id} className={`${T.totalCell} text-right relative border-t-2 border-t-gray-200 ${vline}`}>
                                            {rliq && hasValue && (
